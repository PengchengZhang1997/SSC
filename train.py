...
def train(opt):
    ...
    criterion_id = torch.nn.CrossEntropyLoss().to(device)
    loss_avg = Averager()
    ...
    while (True):
        ...
        for mix_iter in range(opt.inner_loop_iter + 1):
            if mix_iter == opt.inner_loop_iter:
                image_tensors, labels, ids = train_dataset.get_meta_test_batch(meta_target_index)
            else:
                image_tensors, labels, ids = train_dataset.get_batch(meta_target_index)
            image = image_tensors.to(device)
            ids = ids.to(device)
            text, length = converter.encode(labels, batch_max_length=opt.batch_max_length)
            batch_size = image.size(0)
            preds, ee_softmax, ee_log, style_ff, ee_softmax_aug, ee_log_aug, style_ff_aug, scloss, scloss_aug = model(
                image, text[:, :-1])
            target = text[:, 1:]
            cost = criterion(preds.view(-1, preds.shape[-1]), target.contiguous().view(-1))
            id_loss = criterion_id(style_ff, ids)
            log_loss = torch.sum(ee_softmax * ee_log)
            id_loss_aug = criterion_id(style_ff_aug, ids)
        ...
        for key, param in model.state_dict().items():
            if key.find('running') != -1 or key.find('tracked') != -1 or key.find('delta') != -1 or key.find(
                    'hat') != -1:
                new_weights_dict[key] = buffer_buffer[
                    buffer_index]
                buffer_index += 1
                continue
        ...
        preds, ee_softmax, ee_log, style_ff, ee_softmax_aug, ee_log_aug, style_ff_aug, scloss, scloss_aug = model(image,
                                                                                                                  text[
                                                                                                                  :,
                                                                                                                  :-1])
        target = text[:, 1:]
        ...
        if (
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                valid_norm_ED = []
                with torch.no_grad():
                    for num_data in range(len(opt.select_data)):
                        valid_data = os.path.join(opt.valid_data, opt.select_data[num_data])
                        valid_dataset, _ = hierarchical_dataset(root=valid_data, opt=opt)
                        valid_loader = torch.utils.data.DataLoader(
                            valid_dataset, batch_size=opt.batch_size,
                            shuffle=False,
                            num_workers=int(opt.workers),
                            collate_fn=AlignCollate_valid, pin_memory=False)
                        valid_loss, current_accuracy, current_norm_ED, _, _, _, _, length_of_data = validation(
                            model, criterion, valid_loader, converter, opt)
                        valid_acc.append(current_accuracy)
                        valid_norm_ED.append(current_norm_ED)
                    new_valid_acc = sum(valid_acc) / len(valid_acc)
                    new_valid_norm_ED = sum(valid_norm_ED) / len(valid_norm_ED)
                    test_loss, test_accuracy, test_norm_ED, test_preds, test_score, test_labels, test_infer_time, _ = validation(
                        model, criterion, test_loader, converter, opt)
                model.train()

                loss_log = f'[{iteration + 1}/{opt.num_iter}] Train loss: {loss_avg.val():0.5f}, Valid loss: {test_loss:0.5f}, Elapsed_time: {elapsed_time:0.5f}'
                loss_avg.reset()

                if new_valid_acc > best_accuracy:
                    best_accuracy = new_valid_acc
                    best_norm_ED = new_valid_norm_ED
                    test_best_accuracy = test_accuracy
                    test_best_norm_ED = test_norm_ED
                    torch.save(model.state_dict(), f'./saved_models/{opt.exp_name}/best_accuracy.pth')

                now_model_log = f'{"Now_accuracy":17s}: {new_valid_acc:0.3f}, {"Now_norm_ED":17s}: {new_valid_norm_ED:0.3f}'
                best_model_log = f'{"Best_accuracy":17s}: {best_accuracy:0.3f}, {"Best_norm_ED":17s}: {best_norm_ED:0.3f}'
                now_test_model_log = f'{"Now_Test_accuracy":17s}: {test_accuracy:0.3f}, {"Now_Test_norm_ED":17s}: {test_norm_ED:0.3f}'
                best_test_model_log = f'{"Best_Test_accuracy":17s}: {test_best_accuracy:0.3f}, {"Best_Test_norm_ED":17s}: {test_best_norm_ED:0.3f}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--character', type=str,
                        default='0123456789一下东丰云亚亨亮会佑佛佳俊信八关兴凯利力勤华南县发号君和圩城壹大天宁宇安宏宾富封州工帆平年广庆建开德恒惠成新昌明机权来柳桂梧森横永江沙河油泰泽洋浮海润清港湖源滨珠田盈益盛石祥福程穗粤翔肇航良英藤行衡西诚诺谢谷货贵辉达运远途通都金长阳雄韶顺颜风飞香鸿鼎龙',
                        help='character label 0123456789abcdefghijklmnopqrstuvwxyz')
