...
def train(opt):
    ...
    while(True):
        ...
        for mix_iter in range(opt.inner_loop_iter + 1):
            ...
            else: image_tensors, labels, ids = train_dataset.get_batch(meta_target_index)
            image = image_tensors.to(device)
            ids = ids.to(device)
            text, length = converter.encode(labels, batch_max_length=opt.batch_max_length)
            batch_size = image.size(0)
            preds, ee_softmax, ee_log, style_ff, ee_softmax_aug, ee_log_aug, style_ff_aug, scloss, scloss_aug = model(image, text[:, :-1])  # align with Attention.forward
        ...
        preds, ee_softmax, ee_log, style_ff, ee_softmax_aug, ee_log_aug, style_ff_aug, scloss, scloss_aug = model(image, text[:, :-1])
        target = text[:, 1:]
        cost = criterion(preds.view(-1, preds.shape[-1]), target.contiguous().view(-1))
        id_loss = criterion_id(style_ff, ids)
        log_loss = torch.sum(ee_softmax * ee_log)
        ...
        if (iteration + 1) % opt.valInterval == 0:# or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0'
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                for gt, pred, confidence in zip(test_labels[:5], test_preds[:5], test_score[:5]):
